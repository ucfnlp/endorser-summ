...
class SequenceGenerator(nn.Module):
    ...
    def reorder_endorse(self, endorse_scores, new_order):
        """
        ...
        """
        if endorse_scores is not None:
            endorse_scores = endorse_scores.index_select(0, new_order)
        return endorse_scores
    ...
    def _generate(
        ...
        if 'endorse_scores' in sample:
            endorse_scores = sample['endorse_scores']
        else:
            endorse_scores = None
        if 'rouge_scores' in sample:
        ...
        endorse_scores = self.reorder_endorse(endorse_scores, new_order)
        rouge_scores = self.reorder_endorse(rouge_scores, new_order)
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                endorse_scores = self.reorder_endorse(endorse_scores, reorder_state)
                rouge_scores = self.reorder_endorse(rouge_scores, reorder_state)

            lprobs, avg_attn_scores = self.model.forward_decoder(
                tokens[:, : step + 1], encoder_outs, self.temperature, endorse_scores=endorse_scores, rouge_scores=rouge_scores,
            )
...
class EnsembleModel(nn.Module):
    ...
    @torch.jit.export
    def forward_decoder(
        self, tokens, encoder_outs: List[EncoderOut], temperature: float = 1.0, endorse_scores = None, rouge_scores = None,
    ):
        log_probs = []
